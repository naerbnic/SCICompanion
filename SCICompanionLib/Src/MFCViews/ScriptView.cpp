...
void CScriptView::OnContextMenu(CWnd *pWnd, CPoint point)
{
    ...
    if (pTracker)
    {
        ...
        if (_GetInsertObjectMenuItem(pTracker, &insertObjectIndex))
        {
            ...
            if (subMenu)
            {
                _availableObjects = make_unique<AvailableObjects>(appState->GetVersion(), &appState->GetResourceMap().Helper(), GetDocument()->GetScriptId().Language());
        ...
        if (_GetInsertMethodMenuItem(pTracker, &insertMethodIndex))
        {
            ...
            if (subMenu)
            {
                ...
                _availableMethods = make_unique<AvailableMethods>(appState->GetVersion(), &appState->GetResourceMap().Helper(), GetDocument()->GetScriptId().Language());
