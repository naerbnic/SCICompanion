...
void AudioPreferencesDialog::OnBnClickedButtonClearAudioCache()
{
    ...
    {
        std::unique_ptr<AudioCacheResourceSource> cache = std::make_unique<AudioCacheResourceSource>(&appState->GetResourceMap(), appState->GetResourceMap().HelperPtr(), -1, ResourceSourceAccessFlags::ReadWrite);
        cache->Clear();
    }
}
