...
{
    sci::Script script(appState->GetResourceMap().Helper().GetDefaultGameLanguage(), _scriptId);
...
void CNewScriptDialog::OnOK()
{
    ...
    if (fClose)
    {
        ...
        if (!strName.IsEmpty())
        {
            ...
            _scriptId = _scriptId.WithFullPath(appState->GetResourceMap().Helper().GetScriptFileName(_szScriptName));
        }
        else
        {
            fClose = FALSE;
