...
void CMessageDoc::_PreloadAudio()
{
    ...
    std::unique_ptr<AudioCacheResourceSource> resourceSource = std::make_unique<AudioCacheResourceSource>(&map, map.HelperPtr(), mapContext, ResourceSourceAccessFlags::Read);
...
void CMessageDoc::PostSuccessfulSave(const ResourceEntity *pResource)
{
    if (appState->GetVersion().HasSyncResources)
    {
        ...
        if (!audioNegatives.empty())
        {
            ...
            std::unique_ptr<AudioCacheResourceSource> resourceSource = std::make_unique<AudioCacheResourceSource>(&map, map.HelperPtr(), mapContext, ResourceSourceAccessFlags::ReadWrite);
