...
namespace
{
    ...
    void _EnsureCh15Preamble(std::vector<SoundEvent>& events)
    {
    ...
    void WriteChannelStream(const std::vector<SoundEvent>& events, sci::ostream& byteStream, uint8_t channelNumber = 0xff)
    {
        // Now the events.
...
bool SoundEvent::operator == (const SoundEvent& other) const
{
...
void SoundWriteToWorker(const SoundComponent& sound, sci::ostream& byteStream)
{
    ...
    std::vector<std::vector<SoundEvent>> allEvents;
...
void SoundWriteToWorker_SCI1(const SoundComponent& soundIn, const AudioComponent* audio, sci::ostream& byteStream)
{
    ...
    std::vector<std::pair<uint16_t, uint16_t>> channelOffsetAndSize;
    ...
    auto channelInputStream = sci::istream_from_ostream(channelStream);
    // Append the channel data.
    sci::transfer(channelInputStream, byteStream, channelStream.tellp());
}
...
void SoundReadFrom_SCI1(ResourceEntity& resource, sci::istream& stream, const std::map<BlobKey, uint32_t>& propertyBag)
{
    ...
    std::map<uint16_t, int> dataOffsetToChannelId;
    std::map<uint16_t, uint16_t> dataOffsetToSize;   // Temporary, to verify one offset always has same size
...
SoundChangeHint InitializeFromMidi(SCIVersion version, std::vector<DeviceType> devices, SoundComponent& sound, const std::string& filename)
{
        ...
        if (midiFile.is_open())
        {
            ...
            if (dw == 0x4D546864)
            {
                ...
                if (dw == 6)
                {
                    ...
                    {
                        Logger::UserError("Only type 0 and type 1 midi files are supported.");
                    }
                }
                else
                {
                    Logger::UserError("Wrong chunk size");
                }
            }
            else
            {
                Logger::UserError("Not a midi file");
            }
        }
    ...
    {
        Logger::UserWarning("Error opening %s: %s", filename, e.what());
    }
...
void ConvertSCI0ToNewFormat(const std::vector<SoundEvent>& events, SoundComponent& sound, uint16_t* channelsSCI0)
{
    // Given these:
    ...
    std::map<uint8_t, std::set<int>> tracksToUsedChannelNumbers;
    ...
    std::map<int, int> channelNumberToId;
            ...
            case SoundEvent::Special:
                if (bStatus == 0xFF)
                {
                    ...
                    if ((bType == 0x51) && (cbBytes == 3))
                    {
                        if (nTrack != 0)
                        {
                            Logger::DevInfo("Found tempo event in track %d in midi file - ignoring", nTrack);
                        }
                        else
                        {
            ...
            if (_fReEvaluateCues)
            {
                ...
                for (size_t i = 0; i < events.size();)
                {
                            ...
                            (event.bParam1 == 0x60)))
                    {
                        ...
                        // since the std::vector is one smaller now.
                        _RemoveEvent(i, events);
                    }
                    else
                    {
                ...
                std::vector<CuePoint> cuesCopy = Cues;
